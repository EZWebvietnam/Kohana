...
class Collection_List implements IteratorAggregate,ArrayAccess,Countable{
	...
	public function item_at($index)
	{
		...
		else if($index >= 0 AND $index < $this->_count) // in case the value is NULL
			return $this->_data[$index];
		else
		{
	...
	public function insert_at($index, $item)
	{
		if(!$this->_read_only)
		{
			...
			else if($index >= 0 AND $index < $this->_count)
			{
	...
	public function copy_from($data)
	{
		...
		{
			throw new Kohana_Exception('ArrayList data must be an array or an object implementing Traversable.');
		}
		return $this;
	}
